...
public class Exercises {
	...
	public boolean sameFirstLast(int[] nums) {
		...
		if(nums.length >= 1 && nums[0] == nums[lastPosition])
		{
			return true;
		}
		return false;
	...
	public int[] rotateLeft3(int[] nums) {
		...
		nums[lastPosition] = firstElement;
		return nums;
	}
	...
	public int[] maxEnd3(int[] nums) {
		int largestValue = 0;
		int lastPosition = nums.length - 1;
		if (nums[0] > nums[lastPosition]) {
			largestValue = nums[0];

		} else if (nums[0] < nums[lastPosition]) {
			largestValue = nums[lastPosition];
