...
public class Exercise05_AcceptPackage {
    ...
    public boolean acceptPackage(int weightPounds, int lengthInches, int widthInches, int heightInches) {
        int size = lengthInches * widthInches * heightInches;
        if(weightPounds <= 40 && size <= 6912){
            return true;
        }
        else{
            return false;
    ...
    public boolean acceptPackage(int weightPounds, int lengthInches, int widthInches, int heightInches, boolean isSurchargePaid) {
        ...
        int maxDimension = 66;
        if(weightPounds <= 40 && size <= 6912){
            if((lengthInches > maxDimension || widthInches > maxDimension || heightInches > maxDimension) && (isSurchargePaid == true)){
                return true;
            }
            else if((lengthInches > maxDimension || widthInches > maxDimension || heightInches > maxDimension) && (isSurchargePaid == false)){
                return false;
            }
            else{
                return true;
