...
public class Exercise01_ShippingTotal {
    ...
    public double calculateShippingTotal(int weightPounds) {
        double costOf40Pounds = MAX_WEIGHT_POUNDS * UP_TO_40_LB_RATE;
        if (weightPounds <= MAX_WEIGHT_POUNDS && weightPounds >= 0) {
            return (double) (weightPounds * UP_TO_40_LB_RATE);
        } else {
            int additionalWeight = weightPounds - MAX_WEIGHT_POUNDS;
            double costOfAdditionalPounds = (double) additionalWeight * OVER_40_LB_RATE;
            return costOf40Pounds + costOfAdditionalPounds;
        }
    }
    ...
    public double calculateShippingTotal(int weightPounds, boolean hasDiscount) {
        ...
        if (weightPounds <= MAX_WEIGHT_POUNDS && weightPounds >= 0) {
            totalCost = (double) (weightPounds * UP_TO_40_LB_RATE);
        } else {
            int additionalWeight = weightPounds - MAX_WEIGHT_POUNDS;
            double costOfAdditionalPounds = (double) additionalWeight * OVER_40_LB_RATE;
            totalCost = costOf40Pounds + costOfAdditionalPounds;
        }
    ...
    public double calculateShippingTotal(int weightPounds, double discountPercentage) {
        ...
        double discountPrice = totalCost * discountPercentage;
        if (weightPounds <= MAX_WEIGHT_POUNDS && weightPounds >= 0) {
            totalCost = (double) (weightPounds * UP_TO_40_LB_RATE);
            discountPrice = totalCost * discountPercentage;
            totalCostWithDiscount = totalCost - discountPrice;
