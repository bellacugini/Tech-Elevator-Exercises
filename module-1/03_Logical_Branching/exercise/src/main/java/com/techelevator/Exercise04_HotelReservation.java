...
public class Exercise04_HotelReservation {
    ...
    public double calculateStayTotal(int numberOfNights) {
        ...
        if(numberOfNights >= 0 && numberOfNights <= 2){
            totalCost = numberOfNights * DAILY_RATE;
        }
        else{
            totalCost = numberOfNights * DISCOUNT_RATE;
        }
        return totalCost;
    }
    ...
    public double calculateStayTotal(int numOfTotalNights, int numOfWeekendNights) {
        ...
        if(numOfTotalNights > 2){
                numOfTotalNights -= numOfWeekendNights;
                double costOfWeekday = numOfTotalNights * DISCOUNT_RATE;
                double costOfWeekend = numOfWeekendNights * DAILY_RATE;
                totalCost = costOfWeekday + costOfWeekend;
        }
        else{
            totalCost = numOfTotalNights * DAILY_RATE;
        }
        return totalCost;
    ...
    public double calculateStayTotal(int numOfTotalNights, int numOfWeekendNights, boolean isRewardsMember) {
        ...
        if(isRewardsMember == true){
            totalCost = numOfTotalNights * DISCOUNT_RATE;
        }
        else{
            if(numOfTotalNights > 2){
                numOfTotalNights -= numOfWeekendNights;
