...
public class Exercise06_ElectricBill {
    ...
    public double calculateElectricBill(double unitsUsed) {
        ...
        double additionalUnits = 0.0;
        if (unitsUsed <= 100) {
            totalCost = unitsUsed * BASE_RATE;
        } else {
            additionalUnits = unitsUsed - EXCESS_UNITS_LIMIT;
            totalCost = (additionalUnits * EXCESS_RATE) + 20.0;
        }
    ...
    public double calculateElectricBill(double unitsUsed, double unitsReturned) {
        ...
        double total = 0.0;
        double refundRate = 0.2;

        if (netUsage <= EXCESS_UNITS_LIMIT) {
            total = netUsage * BASE_RATE;
        } else {
            double overage = netUsage - EXCESS_UNITS_LIMIT;
            total = (overage * EXCESS_RATE) + (EXCESS_UNITS_LIMIT * BASE_RATE);
        }
